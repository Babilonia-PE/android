...
import com.babilonia.presentation.extension.convertable
import kotlin.properties.Delegates
import kotlin.properties.ReadWriteProperty

const val RANGE_BAR_START_VALUE = 0
const val RANGE_BAR_END_VALUE = 100
...
open class SimpleRangeView @JvmOverloads constructor(
        ...
        when (event.action) {
            MotionEvent.ACTION_MOVE, MotionEvent.ACTION_DOWN -> {
                ...
                if (isRangeMoving && isRange) {
                    val size = end - start
                    var startIndex = closestValidPosition(getPositionByXCoord(x) - linePosToStart)
                    val endIndex = closestValidPosition(startIndex + size)
                    startIndex = endIndex - size

                    if (validatePosition(startIndex) && validatePosition(endIndex)) {
                        start = startIndex
                        end = endIndex
                        invalidate()
    ...
    private companion object DefaultValues {
        ...
        val DEFAULT_LINE_THICKNESS = 2f
        val DEFAULT_ACTIVE_LINE_THICKNESS = 2f
        val DEFAULT_FIXED_LINE_THICKNESS = 2f
