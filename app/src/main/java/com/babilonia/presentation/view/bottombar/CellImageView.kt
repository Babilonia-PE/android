...
internal class CellImageView : AppCompatImageView {
        ...
        try {
            a?.apply {
                ...
                actionBackgroundAlpha =
                    getBoolean(R.styleable.CellImageView_meow_imageview_actionBackgroundAlpha, actionBackgroundAlpha)
    ...
    private fun draw() {
        ...
        if (isBitmap) {
            try {
                val drawable = if (color == 0) {
                    context.getDrawableCompat(resource)
                } else {
                    DrawableHelper.changeColorDrawableRes(context, resource, color)
                }
